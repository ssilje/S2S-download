...
def r_ss(
    ...
    for loc in in_clim.location:
        ...
        fname    = 'se/'+filename+'_'+dim.split('.')[1]+'_'+\
                                name_from_loc(str(loc.values))
        suptitle = title+' Spread-Error '+name_from_loc(str(loc.values))
        ...
        for ii,model in enumerate(in_mod):
            ...
            for n,(xlabel,xdata) in enumerate(x_group):
                ...
                error  = xs.mae(xdata.mean('member',skipna=True),ydata,dim=[])
                spread = xdata.std('member')

                r  = xs.pearson_r(error,spread,dim='time',skipna=True)

                sig_e = xr.ufuncs.sqrt(( ( (xdata-ydata) ** 2 ).sum('member') ))

                g1 = ( sig_e.mean('time',skipna=True) ** 2 )\
                        /( ( sig_e ** 2 ).mean('time',skipna=True) )

                r_ref = np.sqrt(2/np.pi) * ( 1 - g1 )/( 1 - (2/np.pi)*g1 )
...
def skill_agg_cluster(
    ...
    fname    = 'mae_cluster/'+filename+'_'+loc
    suptitle = title+' '+loc
...
def map(
    ...
    for lt in lead_time:
        ...
        fname = 'mae_map/'+filename+'_'+str(lt)
...
def cluster_map(
    ...
    for lt in lead_time:
        ...
        fname = 'mae_cluster_map/'+filename+'_'+str(lt)
