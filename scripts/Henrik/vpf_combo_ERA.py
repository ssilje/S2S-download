...
if verify:
    ...
    hindc = hindc.sel(time=slice(val_obs.time.min(),val_obs.time.max()))

    gr.timeseries(
                    val_obs[var],
                    cast=[clim_fc[var],hindc[var]],
                    title='EC',
                    filename='ppEC_alternative',
                    clabs=['clim','EC']
                )
    gr.timeseries(
                    val_obs[var],
    ...
    crps_mod  = xs.crps_ensemble(val_obs[var],hindc[var],dim=[])
    ...
    gr.skill_plot(crps_mod,crps_clim,title='EC',filename='ppEC_alternative')
    gr.qq_plot(val_obs[var],hindc[var],y_axlabel='EC',filename='ppEC_alternative')
